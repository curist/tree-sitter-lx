...
let s:bvars = {
      \ 'syng_strcom': 'string\|comment\|special\|doc\|template\%(braces\)\@!',
...
" GetLxIndent does to find the containing [[{(] (side-effects)
function s:SkipFunc()
  if s:top_col == 1
    throw 'out of bounds'
  elseif s:check_in
...
function s:OneScope()
  if s:LookingAt() == ')' && s:GetPair('(', ')', 'bW', s:skip_expr)
    ...
    return (count(split('for if let'),tok) ||
          \ s:PreviousToken() ==# 'for') &&
          \ s:Pure('s:PreviousToken') != '.'
  elseif s:Token() =~# '^else$'
...
function GetLxIndent()
  ...
  if b:js_cache[0] > l:lnum && b:js_cache[0] < v:lnum ||
    ...
    try
      ...
      elseif getline(v:lnum) !~ '^\S' && s:stack[-1] =~? 'block\|^lxMap$'
        if !s:GetPair('{','}','bW','s:SkipFunc()') && s:stack[-1] ==# 'lxMap'
  ...
  if !num || s:LookingAt() == '{' && s:IsBlock()
    ...
    if idx == -1 && pline[-1:] !~ '[{;]'
      ...
      elseif num &&
            ...
            \ s:{s:PreviousToken() == '*' ? 'Previous' : ''}Token() !=# 'fn')
        return num_ind + s:sw()
      else
        let is_op = s:sw()
      endif
